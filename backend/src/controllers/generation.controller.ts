...
export async function createGeneration(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (isRoot) {
      ...
      if (!validatedData.imageUrl) {
        return res.status(400).json({ error: "imageUrl is required for original room upload" });
      }
      ...
      if (!parentNode) {
        return res.status(404).json({ error: "Parent generation not found inside this project" });
      }
      ...
      const py = parentNode.y || 350;
      const cx = px + 350;
      
      // Calculate vertical spacing based on current siblings
      const siblingCount = await prisma.generation.count({
        where: { parentId },
      });
      const cy = py - 120 + siblingCount * 240;
...
export async function getGenerationDetail(req: Request, res: Response, next: NextFunction) {
  try {
...
export async function deleteGeneration(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json({ message: "Generation and all its branch descendants deleted successfully" });
  } catch (error) {
    next(error);
  }
}
